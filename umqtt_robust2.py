...
def sub_cb(topic, msg, retained, duplicate):
    ...
    if topic == b'jimm1480/feeds/esp32feed':
        m = msg.decode('utf-8')
        global besked
        besked = m.lower()
        print("\n",besked)
    elif topic == b'jimm1480/feeds/espfeed2':
        m = msg.decode('utf-8')
        global besked2
        besked2 = m.lower()
        print("\n",besked2)
...
ADAFRUIT_IO_FEED2 = credentials["ADAFRUIT_IO_FEED2"]
...
mqtt_feedname2 = bytes('{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, ADAFRUIT_IO_FEED2), 'utf-8')
